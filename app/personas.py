...
def llm_policy_plan(
    ...
    prompt = (
        ...
        "{request_type, reviewed_policy, policy_focus, target_population, variables:[{id,label,categories:[...]}], alternatives:[{label,hypothesis,risk}], "
        ...
        "'노인 돌봄'; each 1-3 words, no region names, no verbs, no sentences. "
...
def simulate_policy_interviews(panel: list[dict[str, Any]], plan: dict[str, Any], seed: int) -> list[dict[str, Any]]:
    ...
    def interview_alternative(policy: dict[str, Any], feedback: str = "") -> list[dict[str, Any]]:
        prompt = (
            ...
            "Write reason, barrier and suggested_change in Korean as SUBSTANTIVE first-person interview content: the persona's "
            "actual stance toward THIS policy, why (tied to their sampled attributes), what concretely blocks them, and what "
            "change would move them. NEVER write meta commentary like '시뮬레이션이다', '추정된다', '단정할 수 없다' — the system "
            "already labels everything as synthetic elsewhere; disclaimers in the answer body destroy the interview's value. "
            "If an attribute is not sampled, simply do not rely on it.\n"
            f"{feedback}"
            ...
            f"All candidate policies in this review (for contrast — you are answering ONLY about the one above): {json.dumps([item.get('label') for item in alternatives], ensure_ascii=False)}\n"
            "Differentiate: the same segment should react differently across policies whose mechanisms differ for its attributes; do not repeat one response pattern across all policies unless they are truly equivalent for that segment.\n"
